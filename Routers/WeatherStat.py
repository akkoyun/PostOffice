...
async def Mobile_App_Root(request: Request, ID: str) -> App_Schema.Model:
	...
	if Last_Update is not None:
		...
		if AT_Data is not None:
			...
			# Parse Max AT Data
			MAX_AT = App_Schema.MaxAT(Value=AT_Data.Max, Time=datetime(AT_Data.Max_Time).strftime("%Y-%m-%d %H:%M:%S"))
			
			# Parse Min AT Data
			MIN_AT = App_Schema.MinAT(Value=AT_Data.Min, Time=datetime(AT_Data.Min_Time).strftime("%Y-%m-%d %H:%M:%S"))
